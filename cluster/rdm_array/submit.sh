#!/bin/bash
# Submits the workers and dependent collector for the RDM wall scan.
...
#!/bin/bash
#SBATCH --job-name=wallscan_rdm_${TASK_ID}
#SBATCH --output=logs/wallscan_rdm-${TASK_ID}-%j.out
#SBATCH --error=logs/wallscan_rdm-${TASK_ID}-%j.err
...
#!/bin/bash
#SBATCH --job-name=wallscan_rdm_collect
#SBATCH --output=logs/wallscan_rdm_collect-%j.out
#SBATCH --error=logs/wallscan_rdm_collect-%j.err
#SBATCH --account=bsc21
#SBATCH --qos=gp_bsccase

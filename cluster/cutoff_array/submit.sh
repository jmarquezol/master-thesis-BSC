#!/bin/bash
# Submits the workers and dependent collector for the cutoff wall scan.
...
#!/bin/bash
#SBATCH --job-name=wallscan_cutoff_${TASK_ID}
#SBATCH --output=logs/wallscan_cutoff-${TASK_ID}-%j.out
#SBATCH --error=logs/wallscan_cutoff-${TASK_ID}-%j.err
...
#!/bin/bash
#SBATCH --job-name=wallscan_cutoff_collect
#SBATCH --output=logs/wallscan_cutoff_collect-%j.out
#SBATCH --error=logs/wallscan_cutoff_collect-%j.err
#SBATCH --account=bsc21
#SBATCH --qos=gp_bsccase

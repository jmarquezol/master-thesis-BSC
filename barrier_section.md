...
**Isolating dial (iii) — and a self-correction worth telling.** The obvious plan was to feed the
symmetric propagator to the symmetric solver (`powermethod_sym` + Takagi). A stress-test of that
plan exposed a subtlety that changes the argument: the XXZ propagator is an exactly symmetric
*operator* ($\|U-U^{\mathsf T}\|\sim10^{-18}$ — inevitable, since our Hamiltonians are real-symmetric)
but is **not manifestly symmetric as an MPO tensor** in the gauge the solver transposes in. The
rotated Hamiltonian's $YY$ layer carries $\sigma^y$, which is transpose-*anti*symmetric, and three
mutually anticommuting operators ($\sigma^x,\sigma^y,\sigma^z$) cannot all be made
transpose-symmetric in any single-site basis (at most two can). So — unlike Ising, whose coupling
$\sigma^x$ *is* symmetric — **XXZ has no single-site-Murg gauge in which the symmetric machinery is
even applicable**; the direct Takagi runs were in an invalid regime and their numbers were
discarded. (This structural fact is itself a result: the symmetric-Takagi route is a privilege of
Ising-like models, not a universally available tool.)

The dial-(iii) conclusion is therefore established by two **gauge-free** routes instead.
*(i) The construction-independence test — decisive.* Run the symmetric-construction Murg propagator
(order 1) through the *ordinary two-sided solver* used for VD2 in notebook 9. Two propagators built
by completely different mathematics — finite-state-machine exponentiation vs analytic
commuting-layer factorization — through the identical solver, reproduce the Rényi-2 dome to 3–4
digits at every $T$, **including the T=4→5 inflation** (peaks 0.22 / 0.39 / 0.70 / 0.82 / 0.97,
matching VD2 to $\lesssim10^{-3}$). The wall is construction- *and* solver-independent.
*(ii) The spectrum bridge* — the gauge-free two-sided block eigensolver on the symmetric tMPO finds
the *same* 4-fold band, at the *same* $T\approx4$, as the asymmetric one ($T=4$:
$|\theta|=[0.864,0.829,0.829,0.826]$ vs $[0.893,0.885,0.867,0.867]$). **The degeneracy is a property
of the Néel quench's transfer matrix, present regardless of construction or solver.** A benign
control seals it: the pure TFIM (Alcaraz $p=0$) run through our *same generic asymmetric pipeline* keeps a
**flat, well-conditioned leading eigenvalue** ($|\lambda_0|\approx1.49$, drift $<1\%$ over $T=2..12$;
**no near-degenerate band forms**) and yields the clean $c=1/2$ calibration — where the quench is
single-sector the spectrum stays benign, so the XXZ band at $T\approx4$ is a property of the *quench*,
not a failure of our adaptation.

**Dial (ii) dominates dial (iii)**: symmetry does not buy back the reach an exact quench degeneracy
takes away — it is either structurally unavailable (Takagi, the $\sigma^y$ obstruction) or
demonstrably irrelevant (the construction test). This sharpens the Ising contrast: Ising's
near-degenerate band is an *emergent, asymptotic* approach to dual unitarity that develops
gradually, leaving a long well-conditioned window; XXZ's is an *exact, structural* degeneracy of the
boundary condition, present from $T=0$ and merely unmasked once other levels decay below it. *(One
loose end, stated honestly: we could not run a correctly-gauged symmetric-Takagi solver, since no
single-site symmetric gauge exists for XXZ; the strong expectation — the same ill-conditioned
eigenvector, since §(ii) shows the band is present in the symmetric tMPO too — is not formally
proved.)*

...
## 0. NOTEBOOK CODE STYLE — WRITE FOR HUMANS (read before writing ANY notebook code cell)

Code cells in `NBs/*.ipynb` are a THESIS DELIVERABLE: the student and supervisor read them like a
lab notebook, not just execute them. Write them the way you'd explain the computation on a
whiteboard, not the way you'd golf a script. This is a standing, PROJECT-SPECIFIC override of the
terse/no-comments default in the global CLAUDE.md ("Token-saving rules" §3/§7) — it applies ONLY to
`NBs/*.ipynb` code cells. `src/*.jl` stays a compact, comment-light library, and chat responses stay
terse as usual; only the notebook cells themselves get the human-friendly treatment.

Concretely, in every notebook code cell you write or rewrite:
- One logical step per line. Don't chain multiple statements with `;` on one line, don't nest
  ternaries, avoid dense one-liner loops/comprehensions when a short, plain `for` loop with named
  intermediates says the same thing more clearly.
- Give intermediate results a descriptive name on their own line, rather than inlining a multi-step
  expression into a single call.
- A short comment explaining WHY a step exists (the physics reason, or which CLAUDE.md section it
  implements) is welcome and encouraged here — unlike `src/`, the notebook is the place for that
  narration.
- Prefer clarity over cleverness even at the cost of a few extra lines: an explicit loop over a
  compact broadcast/`map` one-liner, a spelled-out condition over a packed boolean expression.
- Still REUSE the existing library functions in `src/` (don't reinvent them) — this is about how
  the glue code around those calls (loops, prints, plotting, small helper functions defined in the
  notebook itself) reads, not about rewriting the library.

This supersedes the denser style used in earlier notebook cells (NB3 onward) — new or rewritten
notebook work should read more spelled-out than those.
...
CORROBORATED (July 2026, NB5 fix — see the corrected bullet below) by a SECOND, independent route:
Eq.(3) eigenvalue c-extraction, using the PHYSICAL λ0 (complex-value continuity, `pick_phys`/
`theta_phys`, already stored per-(p,T) in nb8_master.jld2) gives c(p=0.1)=0.50-0.53 STABLE across every
fit window T_min=2..6 — not "contaminated" as earlier versions of this file claimed (see below).
...
real critical-point feature), surfacing as a NUMERICAL wall because:
  ...
  - CORRECTION (July 2026, found while rewriting NB5's human-readable pass): the claim that used to be
    here ("the EIGENVALUE route does NOT rescue c for p=0.1 either... Eq.(3) c swings 0.17-0.98") was
    WRONG, and the bug was entirely in NB5's OWN diagnostic cell, not in the physics. That cell tracked
    λ0 with a SEPARATE, INFERIOR helper (`track_lambda0`, modulus-only nearest-|θ| continuity) instead
    of reading the ALREADY-CORRECT `theta_phys` that NB7's `pick_phys` (full complex-VALUE continuity)
    had computed and cached for every (p,T) all along. Modulus-only tracking cannot distinguish λ0 from
    its near-equal-modulus -λ0 partner (see the "± pairs" bullet in §17's XXZ section and next_steps.md
    for the still-open question of WHY that partner exists) and silently hops onto it once the pair
    tightens (T≳5 for p=0.1) — that hop, not a real ambiguity in the physics, produced the swinging c.
    Re-run with the correct `theta_phys`: |λ0| is smooth and MONOTONE for p=0.1 (no dip-then-rise), and
    the calibrated c(p=0.1)=0.5·|κ(0.1)/κ(0)| is STABLE at 0.50-0.53 across every T_min in [2,6] — a
    clean SECOND, independent confirmation of c≈1/2, not a failure. The eigenVALUE route survives the
    wall; only the eigenVECTOR (hence the entropy) does not. NB5 was corrected accordingly; NB7's
    "Route 2" text was also updated (it separately used the correct `theta_phys` for the λ0 circle, so
    that part was never wrong — only its unverified prose claim that Eq.(3) was "also contaminated" was
    removed/corrected, and NB7 now computes Eq.(3) directly instead of only asserting its failure).
    Eq.(4)'s boundary-exponent x1≈1.5 (vs the free-BC target 1/2) for p=0.1 is a SEPARATE finding and
    was numerically re-checked to NOT be the same selector artifact (its phase-closest λ1 selector
    tracks a stable branch through T=11, only destabilizing at T=12, deep in the wall) — it remains an
    open, likely genuine result, not yet understood; see next_steps.md.

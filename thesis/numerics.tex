...
The transverse contraction is controlled by the leading spectrum of the transfer matrix $\mathcal{E}$, as shown in Eq.~\eqref{eq:contraction}. The numerical problem is therefore to extract its leading eigenvectors while keeping the bond dimension of their \ac{tMPS} representation under control. This becomes more difficult at long times, when the leading eigenvalues approach one another in modulus (Section~\ref{sec:transverse:eigenproblem}). To mitigate this problem, we implement a block power method that evolves several states simultaneously.
...
\end{equation}
Because $\mathcal{E}$ is non-normal (Section~\ref{sec:mporep:fsm}), the left eigenvectors $\bra{L_i}$ and the right eigenvectors $\ket{R_i}$ are different objects, and neither set is orthogonal on its own. The two sets are orthogonal to each other, $\braket{L_i}{R_j}\propto\delta_{ij}$, and this is the pairing we use throughout. The bra notation carries no extra convention here. Each $\bra{L_i}$ is a row object from the start, defined by $\bra{L_i}\mathcal{E}=\mu_i\bra{L_i}$. Pairing it with a column $\ket{R_j}$ is then an ordinary contraction of the network, with no conjugation anywhere. Numerically, the left states are propagated with the transpose $\mathcal{E}^{\mathsf T}$.

We order the complex eigenvalues by modulus and denote the dominant one by $\mu_0$. Its modulus determines the Loschmidt rate, its phase encodes the central charge, and the gap to $\mu_1$ gives access to the boundary operator content. The generalized temporal entropies instead depend on the dominant pair $(\bra{L_0},\ket{R_0})$. The calculation must therefore recover both the eigenvalues and the individual left and right eigenvectors within the leading block. Further implementation details are given in Appendix~\ref{app:blockpm}.
...
The block returns $k$ Ritz values at each evolution time, and the physical $\mu_0$ has to be identified among them. It is normally the one of largest modulus. At $p\neq0$ the block also holds the displaced partner of Section~\ref{sec:results:tower}, whose modulus approaches that of the physical branch and can exceed it, so modulus alone becomes insufficient. The phase still separates the two, because it winds with $T$ at a rate set by the sound velocity. The physical branch advances by a nearly constant amount per rung. Its phase at the next rung can therefore be predicted from the rungs already accepted, and the block member closest to that prediction is selected. Appendix~\ref{app:selector} gives the procedure and the conditions under which it fails.
...
\begin{equation}
    ...
    \label{eq:rigidity}
\end{equation}
the normalized overlap between the left and right eigenvector of the same eigenvalue. For a Hermitian matrix the two coincide and $r_j=1$. As two eigenvectors rotate towards a common direction $r_j$ falls towards zero, and at $r_j=0$ they have coalesced: the matrix is defective and has no complete eigenbasis. Two eigenvalues can approach each other without their eigenvectors coalescing, and the eigenvalues alone do not distinguish the two cases, whereas $r_j$ does. Appendix~\ref{app:wall} develops the consequences.

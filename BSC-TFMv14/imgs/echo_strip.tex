...
\begin{tikzpicture}[
  ...
  \node[tag, tnrow, anchor=east] at (-1.15,{4*\DY}) {$U(\delta t)$};
  \draw[vbox] ({4*\DX-0.32},-0.34) rectangle ({4*\DX+0.32},{8*\DY+0.34});
  \node[tag, tnbox] at ({4*\DX},{8*\DY+0.74}) {$\mathcal{E}$};

  \node[tag, anchor=east] at (-1.0,{8*\DY}) {$\bra{\Psi_0}$};
  \node[tag, anchor=east] at (-1.0,0)       {$\ket{\Psi_0}$};

  \node[tag] at ({\NX*\DX/2},-1.05) {(a)};

  \node[font=\large] at ({\NX*\DX+1.55},{4*\DY}) {$\equiv$};
  ...
  \begin{scope}[xshift={(\NX*\DX+2.85)*1cm}]
    ...
    \node[tag, tnbox] at ({0.68*\SW+0.31},{8*\DY+0.74}) {$\mathcal{E}$};
